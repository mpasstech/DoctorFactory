            ...
            <div class="row">
                <div class="middle-block">
                    ...
                    <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
                        <?php echo $this->element('app_admin_inner_tab_event'); ?>
                        <div class="Social-login-box payment_bx">
                            <?php echo $this->Form->create('Search',array('type'=>'get','url'=>array('controller'=>'admin','action' => 'admin_search_permit_event'),'admin'=>true)); ?>
                                        ...
                                        <table class="table">
                                            ...
                                            foreach ($event as $key => $value){
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <button type="button" id="changeEventPermission" class="btn btn-primary btn-xs"  event-id="<?php echo $value['Event']['id']; ?>" ><?php echo $value['Event']['mbroadcast_publish_status']; ?></button>
                                                    </td>
                                                    <td>
...
<script>
    $(document).ready(function(){
        $(document).on('click','#changeEventPermission',function(e){
            var eventID = $(this).attr('event-id');
            var thisButton = $(this);
            $.ajax({
                url: baseurl+'/app_admin/change_event_mbroadcast_publish_status',
        ...
        $(document).on('click','#viewEvent',function () {
            ...
            var thisButton = $(this);
            $.ajax({
                url: baseurl+'/app_admin/view_event',
                data:{eventID:eventID},
                type:'POST',
                beforeSend:function(){
                    $(thisButton).button('loading').html('<i class="fa fa-spinner fa-pulse fa-2x fa-fw">');
        ...
        $(document).on('click','#getOptionEvent',function () {
            ...
            var thisButton = $(this);
            $.ajax({
                url: baseurl+'/app_admin/get_event_allow_detail',
                data:{eventID:eventID},
                type:'POST',
                beforeSend:function(){
                    $(thisButton).button('loading').html('<i class="fa fa-spinner fa-pulse fa-2x fa-fw">');
                ...
                success: function(result){
                    ...
                    if(result.status == 1)
                    {
                        ...
                        var mediaLinkHref ='<?php echo $this->Html->url(array('controller'=>'admin','action'=>'permit_media_event')); ?>/'+btoa(eventID);
                        var organizerLinkHref ='<?php echo $this->Html->url(array('controller'=>'admin','action'=>'permit_organizer_event')); ?>/'+btoa(eventID);
                        var speakerLinkHref ='<?php echo $this->Html->url(array('controller'=>'admin','action'=>'permit_speaker_event')); ?>/'+btoa(eventID);
                        var agendaLinkHref ='<?php echo $this->Html->url(array('controller'=>'admin','action'=>'permit_agenda_event')); ?>/'+btoa(eventID);
                        var ticketLinkHref ='<?php echo $this->Html->url(array('controller'=>'admin','action'=>'permit_ticket_event')); ?>/'+btoa(eventID);
                        var showLinkHref ='<?php echo $this->Html->url(array('controller'=>'admin','action'=>'permit_show_event')); ?>/'+btoa(eventID);
