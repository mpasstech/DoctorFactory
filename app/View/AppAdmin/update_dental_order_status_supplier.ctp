<?php if(!empty($statusHistory)){ ?>
    ...
    <table class="table">
        ...
        <?php foreach($statusHistory AS $history){ ?>
            <tr>
                <td>
                    <lable><?php echo $history['TeethOrderStatusSupplire']['status']; ?></lable>
                </td>
                <td>
                    <?php echo $history['TeethOrderStatusSupplire']['comment']; ?>
                </td>
                <td>
                    <?php echo date("d/m/Y H:i:s",strtotime($history['TeethOrderStatusSupplire']['created'])); ?>
                </td>
            </tr>
        <?php } ?>
    </table>
...
<?php if($hasForm == 'YES'){ ?>
    ...
    <script>
        ...
        $(document).on("submit","#updateStatusHistoryForm",function(e){
            ...
            $.ajax({
                url: baseurl+'/app_admin/update_dental_order_status_history',
                data:dataToSend,
                type:'POST',
                beforeSend:function(){
