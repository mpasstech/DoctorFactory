...
?>
    ...
    <ul class="slot_blok">
        ...
        <?php } ?>
            ...
            <?php if(!empty($final_array)){
                $queue_number= $time_array = array();$last_time='';
                $break_counter = 0;
                                        ...
                                        <?php if($list['flag']=='BOOKED' || $list['flag']=='RESCHEDULE'){ ?>
                                            ...
                                            <?php if($status=="CONFIRM" || $status=="RESCHEDULE" || $status=="NEW" ) { ?>
                                                ...
                                                <?php if(isset($list['uhid']) && !empty($list['uhid'])){ ?>
                                                    <button type="button"  class="btn history_btn btn-xs btn-warning" src="<?php echo $this->Html->url(array('controller'=>'app_admin','action'=>'get_patient_history',base64_encode($login['User']['thinapp_id']),base64_encode($list['uhid']))) ?>" ><i class="fa fa-money"></i> Patient History</button>
    ...
    <div class="total_earning_div">
        <ul>
                <li class="today_collection_lbl"><strong>Today's Collection  asdasdf</strong></li>
