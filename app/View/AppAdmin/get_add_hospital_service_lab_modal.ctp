...
<div class="form-group">
    <div class="col-sm-12">
        <label>&nbsp;</label>
        <button type="submit" id="updateStatusBtn" class="form-control btn btn-info " >Add</button>
    </div>
</div>
...
<script>
    ...
    $(document).off('submit','#MedicalProductGetAddHospitalServiceLabModalForm');
    $(document).on('submit','#MedicalProductGetAddHospitalServiceLabModalForm',function(e){
        e.preventDefault();
        var dataToSend = $(this).serialize();
        var $btn = $("#updateStatusBtn");
        $.ajax({
            url: baseurl+'/app_admin/add_hospital_service_lab_ajax',
            data:dataToSend,
            type:'POST',
            beforeSend:function(){
