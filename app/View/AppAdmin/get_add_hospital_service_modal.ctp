...
?>
    ...
    <div class="form-group">
        <div class="col-sm-12">
            <label>&nbsp;</label>
            <button type="submit" class="btn btn-info form-control" id="updateStatusBtn" >Add</button>
        </div>
    </div>
...
<script>
    ...
    $(document).off('submit','#MedicalProductGetAddHospitalServiceModalForm');
    $(document).on('submit','#MedicalProductGetAddHospitalServiceModalForm',function(e){
        e.preventDefault();
        var dataToSend = $(this).serialize();
        var $btn = $("#updateStatusBtn");
        $.ajax({
            url: baseurl+'/app_admin/add_hospital_service_ajax',
            data:dataToSend,
            type:'POST',
            beforeSend:function(){
