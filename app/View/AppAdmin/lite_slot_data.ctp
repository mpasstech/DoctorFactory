...
<div class="slot_div" >
    ...
    <?php
    error_reporting( 0 );
    $login =$staff_login=  $this->Session->read('Auth.User');
    ...
    $selected_doctor_data = $this->AppAdmin->get_doctor_by_id(($doctor_id),$login['User']['thinapp_id']);
    ...
    <ul class="slot_blok">
        ...
        <?php if(!empty($final_array)){
            ...
            foreach ($final_array as $key => $list){
                ...
                $walk_in = (isset($list['has_token']) && $list['has_token']=="NO")?' WALK-IN':'';
                $consulting_type = isset($list['consulting_type'])?$list['consulting_type']:'';
                    ...
                    <li class="col-lg-3 col-md-3 col-sm-3 col-xs-3">
                        ...
                        <?php $expire_class =  ( strtotime($booking_date) == strtotime(date('m/d/y')) && strtotime($list['slot']) < strtotime(date('H:i')))?'expire_class':'upcoming_slot'; ?>

                        <?php $booked_status =  false; if($list['status']=='BOOKED' || $list['status']=='CLOSED' || $list['status']=='REFUNDED'){
                        ...
                        <div  <?php echo @($list['emergency_appointment']=="YES")?'emergency_appointment':''; ?> data-time="<?php echo $list['time']; ?>" data-i = "<?php echo base64_encode($list['appointment_id']); ?>" data-slot = "<?php echo base64_encode($list['time']); ?>" queue_number = "<?php echo base64_encode($list['queue_number']); ?>" class="slot_main_div <?php echo @($list['emergency_appointment']=="YES")?'emergency_appointment':''; ?> <?php echo $expire_class; ?> <?php echo ($list['status'] == 'REFUND')?$list['status']:''; echo $list['status'].$walk_in; ?> <?php echo $payStat; ?> " href="javascript:void(0);" >
                                ...
                                <?php if($booked_status){ ?>
                                    ...
                                    <?php }else{ ?>
                                        ...
                                        <?php if($status != 'REFUND'){ ?>
                                            <?php if(($user_role == 'ADMIN' || $staff_data['allow_refund_payment'] =="YES")){ ?>
                                    ...
                                    <?php if($status=="BOOKED") { ?>
                                        ...
                                        <?php if ($login['USER_ROLE'] == 'ADMIN' OR $staff_data['allow_change_appointment_doctor'] == "YES" ) { ?>
                                            <button type="button"  data-di ="<?php echo base64_encode($list['appointment_staff_id']); ?>" data-id ="<?php echo base64_encode($list['appointment_id']); ?>" class="btn btn-xs btn-info btn_assign_doctor"><i class="fa fa-tasks"></i> Assign Doctor</button>
                                        <?php } ?>



                                        <?php if(isset($list['uhid']) && !empty($list['uhid'])){ ?>
                                            <button type="button"  class="btn history_btn btn-xs btn-warning" src="<?php echo $this->Html->url(array('controller'=>'tracker','action'=>'get_patient_history',base64_encode($login['User']['thinapp_id']),base64_encode($list['uhid']),base64_encode($list['appointment_id']))) ?>" ><i class="fa fa-money"></i> Patient History</button>
                                        ...
                                        <?php if ($login['USER_ROLE'] == 'ADMIN' OR $staff_data['allow_reschedule_appointment'] == "YES" ) { ?>
                                            <button type="button" data-st_id ="<?php echo base64_encode($doctor_id); ?>" data-address ="<?php echo base64_encode($address_id); ?>" data-service ="<?php echo base64_encode($service_id); ?>" data-id ="<?php echo base64_encode($list['appointment_id']); ?>" class="btn btn-xs btn-success btn_reschedule"><i class="fa fa-refresh"></i> Reschedule</button>
                                        <?php } ?>

                                        <?php if( ( ($payment_status == "PENDING") || ( @$list['total_billing'] == 0 && $list['booking_validity_attempt'] > 1 )  ) ){  ?>

                                            <button type="button" data-id ="<?php echo base64_encode($list['appointment_id']); ?>" class="btn btn-xs btn-danger cancel_btn"><i class="fa fa-close"></i> Cancel</button>

                                        <?php } ?>
                                        <?php if($consulting_type=='VIDEO' && in_array($role,array('ADMIN','DOCTOR'))){ ?>
                                            <button type="button" data-href="<?php echo Router::url("/homes/video/",true).base64_encode($list['appointment_id']."##DOCTOR"); ?>" data-id ="<?php echo base64_encode($list['appointment_id']); ?>" class="btn btn-xs btn-danger start_video_btn"><i class="fa fa-video-camera"></i> Start Video Call</button>
                                        <?php } ?>

                                        <?php if( $consulting_type=='AUDIO' && in_array($role,array('ADMIN','DOCTOR'))){ ?>
                                            <button type="button" data-href="<?php echo "https://mngz.in:3005/?".base64_encode("ai=".$list['appointment_id']."&m=".$list['mobile']."&n=".$list['patient_name']."&l=".$login['Thinapp']['logo']); ?>" data-id ="<?php echo base64_encode($list['appointment_id']); ?>" class="btn btn-xs btn-danger start_audio_btn"><i class="fa fa-phone"></i> Start Audio Call</button>
                                        <?php } ?>
                                        ...
                                        <?php if($list['mobile'] != '+919999999999'){ ?>
                                            <button type="button" data-pt ="<?php echo ($list['customer_type']); ?>" data-ai ="<?php echo base64_encode($list['appointment_id']); ?>" data-pi ="<?php echo base64_encode($list['customer_id']); ?>"  data-m ="<?php echo $list['mobile']; ?>" class="btn btn-xs btn-info switch_patient_btn"><i class="fa fa-toggle-on"></i> Switch Patient</button>
                                        <?php } ?>
                                    ...
                                    <?php if($list['appointment_status']=="CONFIRM" || $list['appointment_status']=="RESCHEDULE" || $list['appointment_status']=="NEW" ) { ?>
                            ...
                            <div class='user-info-taken' >
                                ...
                                <?php if($booked_status){ ?>
                                    <div class='user-info-taken' >
                                        <label><i class='fa fa-user'></i>&nbsp; <?php echo mb_strimwidth( $list['patient_name'], 0, 23, "..."); ?> </label>
                                ...
                                <?php }else{ ?>
                                    ...
                                    <?php $receptionist_allow = ($role=="RECEPTIONIST" && (( $staff_data['allow_book_past_appointment']=="YES" && strtotime($booking_date) <= strtotime(date('m/d/Y'))) || ($staff_data['allow_book_past_appointment']=="NO" && strtotime($booking_date) == strtotime(date('m/d/Y')))))?true:false; ?>
                                    <?php if($receptionist_allow || in_array($role,array('ADMIN','DOCTOR'))){ ?>
                                        <?php if( $list['sub_token'] != 'NO' && $list['status']!='BLOCKED' ){ ?>
                                    ...
                                    <div class='user-space-token' >
                                        <label><?php echo $list['status']; ?></label>
                                    </div>
                            ...
                            <div class='user-info-taken time_slot_label' data-token = "<?php echo $list['queue_number']; ?>" data-time="<?php echo $list['time']; ?>" data-status = "<?php echo $list['status']; ?>"  >
                                <?php
                                if(!isset($list['emergency_appointment']) || $list['emergency_appointment']=='NO'){
                                    echo $list['time'];
                                }
                                if($list['emergency_appointment']=='YES'){
                                    ...
                                    if($list['status'] == 'CLOSED'){
                                        ...
                                        echo '<b>'.(($list['status'] !== 'AVAILABLE')?"&nbsp;|&nbsp".$list['status']:"").'</b>';
                                    }
                                }
                        ...
                        <?php if($list['status']=='BOOKED' || $list['status']=='RESCHEDULE'){ ?>
                ...
                if ($smart_clinic && $list['break_end']=="YES"){
                    $last_time = $breaks_array[$break_counter];
                    $break_counter++;

                    ?>
                    <li class="col-lg-3 col-md-3 col-sm-3 col-xs-3">
                        <div data-type="WALK-IN"  class="add_walk-in_token slot_main_div break_walk_in"
                             data-last-time="<?php echo base64_encode($this->AppAdmin->get_next_walk_in_time_after_break(($doctor_id),base64_decode($address_id), date('Y-m-d',strtotime($booking_date)),$last_time,$list['time'])); ?>"
            ...
            $new_time = date('h:i A',strtotime("+$interval", strtotime($max_time)));
            ?>
        ...
        <?php
        if(empty($new_time)){
            $new_time = @$this->AppAdmin->get_doctor_address_time(($doctor_id),base64_decode($address_id))['time_from'];
        }
     ...
     */
            ...
            if(!is_desktop()){
                $(".BOOKED, .REFUNDREFUND").closest("li").find('.option_bar_btn').show();
            }

            $(document).off('click','.option_bar_btn');
            $(document).on('click','.option_bar_btn',function () {
                $(this).closest("li").find(".BOOKED, .REFUNDREFUND").trigger('mouseenter');
            });

            $(document).off('click','.close_option_btn');
            $(document).on('click','.close_option_btn',function () {
                $(this).closest("li").find(".BOOKED, .REFUNDREFUND").trigger('mouseleave');
            });


            $(document).off('mouseenter','.BOOKED, .REFUNDREFUND');
            $(document).on('mouseenter','.BOOKED, .REFUNDREFUND',function () {
                if(!is_desktop()){
            ...
            $(document).off('mouseleave','.BOOKED, .REFUNDREFUND');
            $(document).on('mouseleave','.BOOKED, .REFUNDREFUND',function () {
                if(!is_desktop()){
