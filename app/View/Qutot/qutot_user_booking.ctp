        ...
        <?php if(!empty($data)){ ?>
            <div id="token_booking_sec" class="row section_box">
                ...
                <div class="col-12 buton_box" style="padding-left: 0;padding-right: 0px;">
                    <?php if(count($data) > 1){ ?>
                        <button type="button" class="action_btn btn btn-xs button btn-danger" style="display: none;" id="select_doctor"><i class="fa fa-long-arrow-left" aria-hidden="true"></i> Back</button>
                    <?php } ?>

                    <button type="button" class="action_btn btn btn-xs button"  style="display: none;" id="book_appointment">Book Token<span>( Next Available )</span></button>

                    <?php if(!isset($_GET['il'])){ ?>
                    <button type="button" class="action_btn schedule_token_btn btn btn-xs button"  id="schedule_token">Schedule Token</button>
                    <?php } ?>
...
<script>
    $( document ).ready(function() {
        ...
        $(document).on("click",".schedule_token_btn",function(e){
            var di = btoa($('.doctor_input:checked').attr('data-id'));
            var t = "<?php echo base64_decode($thin_app_id); ?>";
            $long_url = baseUrl+"qutot/web_app/"+t+"/?t="+di;
