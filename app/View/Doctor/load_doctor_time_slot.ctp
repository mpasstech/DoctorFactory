...
if(!empty($appointment_slot['slot_list'])){
    ...
    <script>
        $(function () {
            ...
            if(status=='OPEN'){
                var message = "<br>Please book appointments between <br><b>"+address_lbl+"</b>";
                if(is_date_blocked=='YES'){
                    message = "<br>Doctor is not available<br>";
                }
                $(".slot_error_message").html(info+message);
            }else{
                $(".slot_error_message").html(info+"<br>Doctor will open appointment slots shortly.");
            }
        })
