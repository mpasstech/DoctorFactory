...
$query = "SELECT `users`.`firebase_token`,`users`.`thinapp_id` FROM `users`
 ...
 AND `medicine_reminder`.`remind_at_night` = 'YES' AND `medicine_reminder`.`status` = 'ACTIVE'
...
if($totalData->num_rows)
{
	...
	foreach($dataToSend AS $thinappID => $firebaseTokenArr)
	{
		$option = array(
			   ...
			   'title' => "Night Medicine Reminder",
			   'message' => "It's Right Time To Take Your Night Medicine!",
			   'description' => "Night medicine reminder",
			   'chat_reference' => '',
