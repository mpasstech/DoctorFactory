...
$query = "SELECT `users`.`firebase_token`,`users`.`thinapp_id` FROM `users`
 ...
 AND `medicine_reminder`.`remind_at_afternoon` = 'YES' AND `medicine_reminder`.`status` = 'ACTIVE'
...
if($totalData->num_rows)
{
	...
	foreach($dataToSend AS $thinappID => $firebaseTokenArr)
	{
		$option = array(
			   ...
			   'message' => "It's Right Time To Take Your Afternoon Medicine!",
			   'description' => "Afternoon medicine reminder",
			   'chat_reference' => '',
