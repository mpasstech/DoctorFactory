...
(function($)
{
    ...
    var TagSuggest = function(element, options)
    {
        ...
        var self = {
            ...
            /**
             * call when item added
             * @param inputObj
             * @private
             */
            _itemAddedToInput: function() {

                var show_category = false;
                var cat_id = $("#setp_tab .active a").attr('data-id');

                $("#"+cat_id).find('.tag-sel-ctn input[type="hidden"]').each(function () {
                ...
                var inputObject = $(ms)[0].container;
                var all_items = ms.getSelectedItems();
                var append_tags =[];
                $.each(all_items, function( k, v ) {
                    append_tags.push( v.name );
                });
                var inputObject = inputObject[0];
                var step_id = $(inputObject).attr('id');
                if(append_tags.length == 0){
                    $("."+step_id+ ", .th_"+step_id).hide();
                }else{
                    $("."+step_id+ ", .th_"+step_id).show();
                }

                $("."+step_id).html(append_tags.join("</br>"));
