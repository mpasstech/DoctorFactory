...
$rawCallbackData = file_get_contents('php://input');
$phonepay_response = json_decode($rawCallbackData, true);
$phonepay_response =$phonepay_response['response'];
$phonepay_response = json_decode(base64_decode($phonepay_response), true);
$txStatus = $phonepay_response['code'];
$txMsg = $phonepay_response['message'];
$phonepay_data = $phonepay_response['data'];

$orderIdRow = $orderId = $phonepay_data['merchantTransactionId'];
$orderIdArr = explode('-',$orderId);
$orderId = $orderIdArr[0];
$orderAmount = ($phonepay_data['amount']/100);
$referenceId = $phonepay_data['transactionId'];

$paymentMode = isset($phonepay_data['paymentInstrument']['type'])?$phonepay_data['paymentInstrument']['type']:'CASH';
...
else
{
    ...
    $stmt->bind_param('ssss',  $status, $cancel_by_user_id, $currDate, $currDate);
    $result = $stmt->execute();
