...
class Custom
{
    ...
    {
        $file_name = "staff_appointment_address" . $staff_id;
        if (!$data = json_decode(WebservicesFunction::readJson($file_name, "doctor"), true)) {
            ...
            if ($service_message_list->num_rows) {
                ...
                foreach ($data as $key => $val) {
                    $temp_data[$val["appointment_address_id"]] = $val;
                }
                $data = $temp_data;
    ...
    public static function create_block_slot_string($slot_string)
    {
        ...
        $time_stamp = array();
        foreach ($slot_string as $key  => $value) {
...
WHERE `messages`.`thinapp_id` = '" . $thinappID . "' GROUP BY `messages`.`id` ORDER BY `messages`.`id` DESC LIMIT 8";
    ...
    public static function getBase64ImageSize($base64Image, $size_in = 'MB')
    ...
    {
        return array(
            "N/A", "TABLET", "CAPSULE", "SYRUP", "DRY SYRUP", "LIQUID", "AMPOULE", "INJECTION", "PRE-FILLED SYRINGE", "CREAM", "LOTION", "OINTMENT", "GEL", "VIAL",
                ...
                if (!$list->num_rows) {
                    ...
                    if ($list->num_rows) {
                        ...
                        if (!empty($result) && !in_array(false, $result)) {
                            ...
                            echo "Doctor IVR setup successfully\nDoctor Code Is : " . $doctor_ivr['doctor_code'];
                            die;
                        } else {
                            echo "Unable to setup Doctor Ivr";
                            die;
                        }
                    } else {
                        echo "Unable to find master step data";
                        die;
                    }
                } else {
                    echo "Doctor already setup ivr doctor code is :" . $doctor_ivr['doctor_code'];
                    die;
                }
            } else {
                echo "Unable to setup Doctor Ivr";
    ...
    public static function getStaffDayTimeData($staff_id, $day_time_id)
    {

        $file_name = "staff_hours_" . $staff_id;
        if (!$staff_day_time = json_decode(WebservicesFunction::readJson($file_name, "doctor"), true)) {
            ...
            if ($staff_day_time->num_rows) {
                ...
                $staff_day_time = $temp_data;
                WebservicesFunction::createJson($file_name, json_encode($staff_day_time), "CREATE", "doctor");
            }
        }
